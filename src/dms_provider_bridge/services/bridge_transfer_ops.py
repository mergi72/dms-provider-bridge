...
def max_cross_connection_upload_bytes(connection_runtime) -> int:
...
def max_cross_provider_upload_bytes(connection_runtime) -> int:
    """Backward-compatible alias for provider-era imports."""
    return max_cross_connection_upload_bytes(connection_runtime)


def max_inline_upload_bytes(connection_runtime) -> int:
    transfer_limit = max_cross_connection_upload_bytes(connection_runtime)
    config = getattr(connection_runtime, "config", {})
    if isinstance(config, dict):
...
def ensure_folder_chain(connection_runtime, target_folder_path: str, auth: BridgeAuthContext) -> None:
    ...
    for part in normalized.strip("/").split("/"):
        ...
        existing = connection_runtime.stat_item(current, auth)
        if existing is None:
            connection_runtime.make_dir(current, auth)
            continue
        if not existing.is_folder:
            raise TransferPrecheckError(f"Upload failed: destination segment is not a folder: {current}")


def upload_with_preflight(connection_runtime, destination_path: str, file_name: str, auth: BridgeAuthContext, content_base64: str | None = None, source_path: str | None = None, overwrite: bool = False, versioning: dict | None = None) -> OperationResult:
    if source_path:
        try:
        ...
        max_bytes = max_inline_upload_bytes(connection_runtime)
    ...
    ensure_folder_chain(connection_runtime, destination_path, auth)
    kwargs = {
    ...
    return connection_runtime.upload_item(destination_path, file_name, **kwargs)
