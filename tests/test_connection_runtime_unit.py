...
import dms_provider_bridge.services.connection_runtime_service as connection_runtime_module
import dms_provider_bridge.services.provider_service as legacy_runtime_module
...
def test_get_connection_runtime_accepts_trailing_colon() -> None:
    connection = connection_runtime_module.get_connection_runtime("edocat:")
    assert connection.name == "edocat"


def test_legacy_provider_service_exports_connection_runtime() -> None:
...
def test_runtime_registry_snapshot_maps_wfx_connections(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(connection_runtime_module, "_DRIVER_FACTORIES", {"alfresco": _DummyProvider, "edocat": _DummyProvider})
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: ["alfresco", "edocat"])
    monkeypatch.setattr(connection_runtime_module, "connection_driver_name", lambda name: name)
    monkeypatch.setattr(
        connection_runtime_module,
...
def test_list_registered_connections_wraps_connection_config(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: ["company"])
    monkeypatch.setattr(connection_runtime_module, "connection_driver_name", lambda name: "alfresco" if name == "company" else None)

    assert connection_runtime_module.list_registered_connections() == ["company"]


def test_reload_connection_runtime_cache_clears_cached_instances(monkeypatch: pytest.MonkeyPatch) -> None:
    connection_runtime_module.reload_connection_runtime_cache()
    ...
    connection_runtime_module.reload_connection_runtime_cache()
    third = connection_runtime_module.get_connection_runtime("dummy")
...
def test_driver_factories_use_current_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(connection_runtime_module, "_DRIVER_FACTORIES", factories)

    assert connection_runtime_module._driver_factories() is factories


def test_get_default_connection_name_uses_env_when_registered(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(connection_runtime_module, "load_config", lambda: {})
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: [])
    monkeypatch.setenv("DMS_PROVIDER_DEFAULT_CONNECTION", "alfresco")

    default_connection = connection_runtime_module.get_default_connection_name()
...
def test_get_default_connection_name_requires_configured_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ConfigurationError):
        connection_runtime_module.get_default_connection_name()


def test_get_default_connection_name_uses_config_when_registered(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(connection_runtime_module, "load_config", lambda: {"connection": {"default": "alfresco"}})
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: [])
...
def test_list_registered_connections_prefers_connections(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: ["company", "unknown"])
    monkeypatch.setattr(connection_runtime_module, "connection_driver_name", lambda name: {"company": "alfresco"}.get(name))
...
def test_get_connection_runtime_instantiates_connection_driver(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(connection_runtime_module, "_DRIVER_FACTORIES", {"dummy": DummyProvider})
    monkeypatch.setattr(connection_runtime_module, "list_connection_config_names", lambda: ["mount1"])
    monkeypatch.setattr(connection_runtime_module, "connection_driver_name", lambda name: "dummy" if name == "mount1" else None)
    monkeypatch.setattr(connection_runtime_module, "load_connection_config", lambda name: {"driver": "dummy", "base_url": "x"})

    provider = connection_runtime_module.get_connection_runtime("mount1")
...
def test_get_connection_runtime_aliases_connection_driver_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runtime = connection_runtime_module.get_connection_runtime("mount1")
